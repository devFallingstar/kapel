...
export const RoutingRuleSchema = z.object({
  ...
  complexity: z.array(z.enum(["trivial", "normal", "complex", "architectural"])).default([]),
  agent: z.string(),
...
export const ReviewRuleSchema = z.object({
  id: z.string(),
  riskCategories: z.array(z.string()).default([]),
  minimumComplexity: z.enum(["trivial", "normal", "complex", "architectural"]).optional(),
  reviewer: z.string(),
...
export function validatePolicy(
  ...
  for (const agent of referenced) {
    if (!knownAgents.has(agent)) {
      issues.push({ severity: "error", code: "UNKNOWN_AGENT", message: `Policy references unknown agent: ${agent}` });
    }
  }

  for (const rule of policy.escalation) {
    if (rule.fromAgent === rule.toAgent) {
      issues.push({ severity: "error", code: "SELF_ESCALATION", message: `Escalation ${rule.id} points ${rule.fromAgent} to itself.` });
    }
  }
  return issues;
}
