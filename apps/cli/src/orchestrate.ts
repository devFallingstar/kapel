...
import type { StatusLine, StatusLineStream } from "./status-line.js";
...
export interface OrchestratePresentation {
  ...
  /**
   * Handed the block the run paints its row in, for as long as the run lasts,
   * and `undefined` once that block is down.
   *
   * The other half of the same bargain: the caller says how to paint, and gets
   * back a handle on what is being painted, because a REPL still prints lines
   * of its own while a run works — a line typed mid-run is answered with
   * "queued — runs after this turn" the moment it is Entered. Those notices go
   * through the caller's renderer, which knows how to erase its *own* status
   * block before writing and put it back afterwards, and knows nothing about
   * this one. Without this handle the notice lands on the row the run is
   * painting, and the two smear.
   */
  readonly onBand?: (band: StatusLine | undefined) => void;
}
...
export async function executePreparedPlan(
  ...
  // The row goes up and comes down through these two, never through
  // `workers` directly, so the handle the caller holds on the block (see
  // {@link OrchestratePresentation.onBand}) can never outlive it: a caller
  // erasing rows that are already gone would take a line of the transcript
  // with them.
  const openWorkerRow = (): void => {
    if (workers === undefined) return;
    workers.open();
    presentation.onBand?.(workers.status);
  };
  const closeWorkerRow = (): void => {
    if (workers === undefined) return;
    workers.stop();
    presentation.onBand?.(undefined);
  };
