 ...
 * delegated backend (`--backend codex`) or in a child worker, neither of which
 * reports token usage back to this process. That is distinct from `n/a` in the
 * `$` column, which means the tokens *are* known and their price is not.
 */
function summaryRow(
  task: RuntimeTask,
