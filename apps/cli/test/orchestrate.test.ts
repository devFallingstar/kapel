...
describe("kapel orchestrate", () => {
  ...
  it("lends the row's block to the caller for exactly as long as it is up", async () => {
    const stream = new CapturingStream();
    const { output } = capture();
    ...
    await runOrchestrate("add a health endpoint", options(workspace), {
      ...
      presentation: {
        ...
        tty: true,
        // A REPL keeps this handle so its own mid-run notices can take the
        // row down before they print — see `interactive.ts`.
        onBand: (block) => handed.push(block === undefined ? undefined : "up"),
      },
    });

    // Handed over once when the row goes up, and taken back once when it comes
    // down: a handle on a block that is no longer painted would erase rows
    // belonging to the transcript.
    expect(handed).toEqual(["up", undefined]);
  });
