...
```bash
npm install -g https://raw.githubusercontent.com/devFallingstar/kapel/main/release/devfallingstar-kapel-0.8.1.tgz
kapel --version
```

npm 레지스트리 배포 후에는 `npm install -g @devfallingstar/kapel` 로 대체됩니다.
URL 접근이 안 되는 네트워크라면: 레포를 클론한 뒤
`npm install -g ./kapel/release/devfallingstar-kapel-0.8.1.tgz`. 제거는
...
배너(`kapel v0.8.1  claude-sonnet-5  session 0f3c9a2b`)와 `kapel>` 프롬프트가
...
배너가 `kapel v0.8.1  claude-code · opus  session 0f3c9a2b` 형태로 뜨고, 그
